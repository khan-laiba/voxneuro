...
The dataset-provided binary gender covariate is mapped by the rank normalizer to two values near +-5.2, so in the
full-space Euclidean view it carries a larger weight than a continuous coordinate (in the supervised-subspace branch
its influence depends on the fitted PLS rotations). This script quantifies its incremental value by re-running the
adaptive model, the five standard comparators, and the rank-normalized full-space kernel models (all under rank
normalization) with the covariate removed from the feature set, at the reference partition (seed 42) and across the 20 alternative partitions (seeds 0-19), and pairing
