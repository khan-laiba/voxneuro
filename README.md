...
- at least `--splits` subjects from each class; the PLS-DA comparator additionally uses an inner stratified split with `min(5, smallest training class count)` folds, so at least two training subjects per class are required.
